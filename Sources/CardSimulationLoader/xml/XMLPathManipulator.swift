...
extension XMLPathManipulatorHolder: XMLPathManipulator {
    ...
    public func manipulate(path: XMLPath, with element: AEXMLElement) -> AEXMLElement {
        return self.manipulator(path, element)
    }
}
...
extension XMLPathManipulatorHolder {
    ...
    public static func tlvPortManipulator(port: String) -> XMLPathManipulator {
        return XMLPathManipulatorHolder(path: self.TLVPortXMLPath) { _, element in
            element.value = port
            return element
        }
    }
    ...
    public static func relativeToAbsolutePathManipulator(with path: XMLPath, absolutePath: URL = FileManager.default
        ...
        return XMLPathManipulatorHolder(path: path) { _, element in
