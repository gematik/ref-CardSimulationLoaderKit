...
extension SimulationManager {
    struct DependencyInfo {
        ...
        var pom: URL {
            return simulatorPath.appendingPathComponent("pom.xml")
        }

        var script: URL {
            return simulatorPath.appendingPathComponent("runMaven.sh")
        }

        var simulatorExists: Bool {
            return FileManager.default.fileExists(atPath: simulatorPath.absoluteURL.path)
        }
    }
            ...
            return try AEXMLDocument(xml: pomXml)
                ...
                .flatMap { dependencyInfo in
                    ...
                    return Result {
                        ...
                        // Save POM
                        return xmlData.save(to: dependencyInfo.pom)
                                .flatMap { _ -> Result<DependencyInfo, Swift.Error> in
                                    Result {
                                        ...
                                        // Save runMaven.sh
                                        return scriptData.save(to: dependencyInfo.script).flatMap { _ in
                                            Result {
