...
extension SimulationProcessMode: Equatable {
    ...
    public static func ==(lhs: SimulationProcessMode, rhs: SimulationProcessMode) -> Bool {
        switch lhs {
        case .running(let onTCPPort): return rhs.tlvPort == onTCPPort
        case .notStarted: return rhs.isNotRunning
        case .initializing: return rhs.isInitializing
        case .terminated(let terminationStatus): return rhs.terminationStatus == terminationStatus
        }
    }
...
extension SimulationProcessMode {
    ...
    public var tlvPort: Int32? {
        switch self {
        case .running(let onTCPPort): return onTCPPort
        default: return nil
        }
    }
    ...
    public var terminationStatus: Int32? {
        switch self {
        case .terminated(let terminationStatus): return terminationStatus
        default: return nil
        }
    }
}
...
extension SimulationProcessMode: CustomStringConvertible {
    ...
    public var description: String {
        ...
        case .running(let port): return "Running on TCP: [\(port)]"
        case .terminated(let terminationStatus): return "Terminated (exit: [\(terminationStatus))]"
        }
    }
}
